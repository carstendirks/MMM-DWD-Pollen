...
Module.register("MMM-DWD-Pollen", {
  ...
  getDom: function () {
    function writePollen(
      ...
      function designPollen(tableElement, pollenValue, iconPollen) {
        // Determine color:
        // Low (0-1) and (1) -> Green
	...
	td1.innerHTML = "Beifu&szlig";
      ...
      if (timestamp.getDay() === new Date().getDay() - 1) {
        designPollen(td2, pollenTomorrow, iconPollen);
      } else {
        designPollen(td2, pollenToday, iconPollen);
      }
      ...
      if (timestamp.getDay() === new Date().getDay()) {
        designPollen(td3, pollenTomorrow, iconPollen);
      } else {
        // when data is not from tomorrow then display '-'
        designPollen(td3, pollenDayafter_to, iconPollen);
      }
      
      const td4 = document.createElement("td");
      if (timestamp.getDay() === new Date().getDay()) {
        designPollen(td4, pollenDayafter_to, iconPollen);
      } else {
        // when data is not from day after tomorrow then display '-'
        designPollen(td4, "-", iconPollen);
      }
  ...
  getData: function () {
    ...
    this.sendSocketNotification("DWD_POLLEN_REQUEST", url);
  },

  socketNotificationReceived: function (notification, payload) {
    console.log("[MMM-DWD-Pollen] Frontend hat Notification erhalten:", notification);
    if (notification === "DWD_POLLEN_RESULT") {
      console.log("[MMM-DWD-Pollen] Ergebnis empfangen:", payload);
      this.result = payload;
      this.loaded = true;
